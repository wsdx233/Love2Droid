...
- 游戏调试悬浮球、固定监视 HUD 和自适应调试控制台已实现；黑色半透明悬浮层配白色 Material 图标，面板在横屏从左侧滑出、竖屏从底部滑出，并通过 `Window.adjustPan` 把当前输入框抬到输入法上方。控制台提供日志筛选搜索、清空、无焦点抢占的滚动锁定和 Lua 高亮 REPL；监视表达式可固定到 HUD。Lua 编辑器支持点击行号切换项目持久化断点，Play 时传入 runtime；断点页提供执行控制、调用栈及高亮当前执行行的只读源码视图，不再编辑断点。
