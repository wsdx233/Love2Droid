...
- 已提供中英文双文件 README、五张真实界面截图与统一应用品牌；normal 应用与 README 已同步用户优化后的 Android 抱着 LÖVE 矢量图标及 512px PNG，支持 API 26+ 自适应、API 33+ 单色主题和 API 23+ 兼容图标。优化版已通过 Chromium 图形预览、单色镂空检查、普通 Debug APK 构建与包内资源检查；构图及来源分别见 [design.md](design.md#应用品牌与启动图标) 与 [reference.md](reference.md#应用品牌与-readme-素材)，桌面裁切与主题图标仍需真机确认。
...
- 无当前项目时停留在带 App 图标和无表情欢迎文案的首页，不再自动弹出项目管理；提供导入、新建、打开项目三个入口并复用现有流程。已有项目的无标签欢迎页保留文件操作，内容可滚动并跟随应用主题。普通 Debug APK 构建、23 项项目存储/更新聚焦测试及包内欢迎资源检查通过；页面布局与按钮交互仍需真机验收。
- 新建项目使用独立模板选择与信息填写页面，支持空项目、基础项目，默认选择基础项目。空项目仅提供使用引擎内置字体的 Hello World，不添加 `assets/`、字体或 `AGENTS.md`；两种模板共用项目配置、LuaLS 配置和元数据，创建成功后直接打开编辑器。
- 基础项目离线携带固定版本的 Fusion Pixel 12px 等宽 `zh_hans` OTF 和完整随包许可证，沿用原入口以 24px、`nearest` 过滤启用；创建及字体复制在后台执行，字体随 `.love` 和游戏 APK 分发。已有及导入项目不自动注入字体，来源见 [reference.md](reference.md#fusion-pixel-font)。
- 基础项目自动生成中文 `AGENTS.md`，说明 LuaJIT 语法检查、`love-check` 使用流程及 Linux LÖVE 11.5 / Android 12.0 验证边界；空项目、已有及导入项目不自动补写。
...
- 设置已提供默认开启的自动更新检查开关、当前版本和手动检查入口；每次冷启动静默查询 GitHub 正式 Release，仅更高版本弹 Dialog，自动失败不打扰，手动检查反馈结果。下载交给 Release 页面，不自动安装。9 项版本/通知状态聚焦测试、真实 GitHub 检查与普通 Debug APK 构建已通过；弹窗、浏览器跳转和开关持久化的 Android 交互仍需真机验收。
...
- 单击活动标签或长按任意文件、终端、DSH 标签正文可打开同一标签操作菜单；长按非活动标签不先切换内容，关闭范围以触发标签为准。普通 Debug APK、19 项存储/文件聚焦测试及 7 个主机回调冒烟场景通过；主机脚本使用真实 `EditorSession` 和源码回调、菜单边界替身，不代表 Android 手势或弹窗已完成真机验证。
- AppBar 与 overflow 已按活动标签更新：文件操作仅在文件标签显示；Play/打包按当前项目显示，全局新建、设置和项目管理保留。保存按现有路径/项目条件启用，撤销遵守标签只读状态。普通 Debug APK、24 项存储/文件聚焦测试与 9 个主机菜单状态冒烟场景通过；临时脚本使用真实标签模型及源码状态更新函数，UI 和终端会话边界使用替身，Android 实际显示与点击仍需真机验收。
