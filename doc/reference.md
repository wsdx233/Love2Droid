...
- 离线包源码获取：镜像内 `/usr/local/share/love2droid/debian-packages.tsv` 每行四列依次为二进制包名、二进制版本、源码包名和源码版本。在启用 Ubuntu 24.04 `noble`、`noble-updates`、`noble-security` 对应 `deb-src` 软件源的环境中，按后两列执行 `apt-get source '源码包名=源码版本'`；若普通镜像已不提供该版本，使用 [Ubuntu Launchpad 源码包页面](https://launchpad.net/ubuntu/+source) 按同一包名和版本获取 `.dsc` 及其引用的原始源码、发行版补丁，不以其他版本替代。非 APT 组件按版本锁及本文上游链接获取对应 tag/提交的源码；发布说明应链接到该发行 tag 下的本节，避免后续版本覆盖来源记录。
