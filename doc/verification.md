...
- 打开游戏调试悬浮球，确认黑色半透明 scrim、白色文字、Material 图标和按压 Ripple；竖屏面板从底部滑出并显示纯图标横向 Tab，横屏面板从左侧滑出并显示左侧纯图标竖直导航，切换页面后旋转仍保持当前页面，关闭后游戏仍可操作。面板入场前不得在最终位置闪现一帧。
...
- 在监视页添加 Lua 表达式，确认运行时结果刷新、错误不崩溃，固定项在面板关闭后显示于黑色半透明 HUD；旋转后 HUD 与悬浮球仍在安全区域。
...
- 顶栏暂停恢复按钮随运行时暂停状态切换图标和功能；面板使用 `open_in_full`/`close_fullscreen` Material Symbols 切换全屏，退出游戏和关闭按钮均为图标按钮，点击白色面板内容不会关闭面板。
...
- 在设置中分别切换应用主题和编辑器主题的亮色、暗色、跟随系统，确认两者可以独立组合；系统明暗变化后，跟随系统的选项刷新，重启应用后选择仍保留，普通应用页面和系统栏图标对比度正常。
- 在亮色编辑器中确认代码区、标签栏、快捷输入栏和选区浮动菜单不再固定为黑色；定义和用法图标与 Sora 内置选区菜单图标保持一致，点击后确实发起对应导航。
