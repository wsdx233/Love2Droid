...
- 在亮色编辑器中确认代码区、标签栏、快捷输入栏和选区浮动菜单不再固定为黑色；切换“符号栏”后显示状态立即变化且重启后保留。分别对两个文件标签开启/关闭“只读”，确认只能修改非只读标签，切换标签后各自状态不串用，关闭应用后恢复可编辑。文件标签活动时，保存和撤销图标直接显示在顶栏，窄屏时标题被压缩而操作按钮不进入 overflow。
- 文件 → 普通终端 → OMP → DSH → 文件逐一切换：非文件标签隐藏保存、撤销、另存为、查找和替换、自动换行、符号栏、只读和 LuaLS 悬浮信息，返回文件恢复这些操作及正确勾选状态；只读文件禁用撤销但仍可保存、另存为及关闭只读。切换两个只读状态不同的文件，菜单不串用状态；关闭全部标签后不残留文件操作，长按非活动标签不改变顶栏上下文。
- 当前项目存在时，在文件、终端、OMP、DSH 和无标签页面均可使用 Play/打包，目标不随标签变化；无当前项目时隐藏这两项。新建、设置和项目管理在上述页面始终保留。无项目的未命名文件禁用保存和另存为，有路径的外部文件可保存回原位置但不能另存为；已有项目的未命名文件可通过保存输入项目内文件名。
- 菜单状态改动本地执行 `CMAKE_BUILD_PARALLEL_LEVEL=2 ./gradlew :app:assembleNormalNoRecordDebug :app:testNormalNoRecordDebugUnitTest --tests top.wsdx233.love2droid.StorageAndPackagingTest --tests top.wsdx233.love2droid.EditorFileAccessTest --tests top.wsdx233.love2droid.EditorFileRobustnessTest`，24 项通过。临时 JVM 脚本使用真实 `EditorSession`/标签模型和源码中的 `updateEditorMenuState()`，覆盖空状态、项目状态、文件/非文件切换、只读恢复、外部/未命名保存条件及清空工作区，共 9 个场景通过；菜单、编辑器设置和终端会话边界使用替身，不执行 Android runtime，不代表顶栏布局、overflow 及点击已完成真机验证。
