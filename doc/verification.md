...
- 打开游戏调试悬浮球，确认黑色半透明 scrim、白色文字和 Material 图标；竖屏面板从底部滑出，横屏面板从左侧滑出，关闭后游戏仍可操作。
- 分别在横屏和竖屏聚焦日志搜索、监视输入和 REPL，确认系统窗口只平移到当前输入框位于虚拟键盘上方，输入框不会被遮挡；锁定日志滚动并持续输出日志，当前输入框焦点和已输入内容不被刷新重置。
- 在控制台页确认 `print` 增量出现，等级筛选、关键字搜索、清空、滚动锁定和 REPL 高亮输入可用；REPL 快捷代码与符号栏不显示，语法错误仅进入日志。
...
- 顶栏暂停恢复按钮随运行时暂停状态切换图标和功能；全屏、退出游戏和关闭按钮均为图标按钮，点击白色面板内容不会关闭面板。
